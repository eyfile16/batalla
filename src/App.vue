...
export default {
  setup() {
    ...
    const setFightingPokemon = (pokemon) => {
      // Aquí puedes manejar la lógica de selección de estadísticas para la pelea
    ...
    const nextRound = () => {
      ...
      if (currentRound.value < rounds.value) {
        ...
        winner.value = null; // Reseteamos el ganador
        determineWinner();
      } else {
        endBattle();
      }
    ...
    const endBattle = () => {
      ...
      finalWinner.value = winner.value.name; // Puedes agregar lógica para determinar el ganador final
    };
    ...
    const getPokemonTypeColor = (pokemon) => {
      ...
      // Define un color por tipo (puedes personalizar esto)
      const colors = {
        fire: '#F08030',
        water: '#6890F0',
        grass: '#78C850',
      ...
      return colors[types[0]] || '#FFF'; // Retorna un color por el primer tipo o blanco
    };
